...
class MaskTokensDataset(BaseWrapperDataset):
    ...
    def __init__(
        ...
        if data_path is not None:
            f_read = open('%s/tokens_mask.pkl'%data_path, 'rb')
            self.token_mask = pickle.load(f_read)
            f_read.close()
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            try:
                ...
                assert (mask.size==item.shape[0]), "saved token_mask does not match the size in ids-{}.".format(index)
                num_mask = int(
