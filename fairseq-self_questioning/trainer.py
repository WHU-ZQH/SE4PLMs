...
class Trainer(object):
    ...
    def sentence_decode(self, tokens: torch.LongTensor):
        assert tokens.dim() == 1
        tokens = tokens.numpy()
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output, loss_all, preds = self.task.train_step(
                        sample=sample,
                        model=self.model,
                ...
                ### calculate the mask, geometric distribution ##
                mask=torch.full_like(sample["net_input"]["src_tokens"], 0).bool()
                for ids, pos in (loss_all>self.bar).nonzero().tolist():
                    # num=np.random.choice(self.lens, p=self.len_distrib)
                    # mask[ids][pos-int(num/2):pos+int(num/2)+1]=True
                    mask[ids][pos]=True
                
                for i, idx in enumerate(sample["id"].tolist()):
                    self.token_mask[idx]=mask[i].cpu().nonzero().squeeze().numpy()
                    # self.token_mask[idx]=mask[i].cpu().nonzero().squeeze()

                '''
                unk_ind=torch.full_like(sample["target"], self.task.source_dictionary.unk_index)
                new_input=torch.where(loss_all<1, sample["target"], unk_ind)
                pad=torch.full_like(sample["target"], 0).float()
                loss_filter=torch.where(loss_all>=1, loss_all, pad)
                self.sentence_decode(new_input[1].cpu())[0]
                self.sentence_decode(sample["target"][1].cpu())[0]
                '''
